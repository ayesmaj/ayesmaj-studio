...
/**
 * StarBackground — decorative SVG star-dot pattern rendered inside the button border.
 * color defaults to gold (#C8A44E) to match the AYESMAJ palette.
 */
...
/**
 * StarButton — animated light-sweep button with star-dot border pattern.
 * Themed to AYESMAJ dark green + gold palette.
 *
 * Props:
 *   children        ReactNode  — button label
 *   lightWidth      number     — width of the traveling light orb in px (default 110)
 *   duration        number     — animation duration in seconds (default 3)
 *   lightColor      string     — color of the traveling light (default gold #C8A44E)
 *   backgroundColor string     — star pattern fill color (default gold #C8A44E)
...
export function StarButton({
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        'relative z-[3] overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-semibold transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button',
        'bg-[#07100A] text-[#F2EDE4]',
        className,
      )}
      {...props}
    >
      ...
      {/* Border layer with star pattern */}
      <div
        className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] border-[rgba(200,164,78,0.5)]"
        style={{ borderWidth: 'var(--border-width)', borderStyle: 'solid' }}
        aria-hidden="true"
      >
      ...
      <span
        ...
        style={{
          background: 'linear-gradient(to top, #C8A44E, #E8C96D)',
