...
export default function BrandingCaseStudy() {
  ...
  if (!project) {
    return (
      <div style={{ background: '#0B0F0C', minHeight: '100vh' }} className="flex items-center justify-center">
      ...
      <main className="relative z-10 pt-28 pb-0">
        ...
        <div className="relative w-full" style={{ height: '60vh', minHeight: 360 }}>
          ...
          <div className="absolute inset-0"
            style={{ background: 'linear-gradient(to top, #0B0F0C 0%, rgba(11,15,12,0.5) 50%, rgba(11,15,12,0.2) 100%)' }} />
          <div className="absolute inset-0"
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.04, borderColor: 'rgba(200,163,78,0.6)' }}
            whileTap={{ scale: 0.97 }}
          >
          ...
          <div className="absolute bottom-8 left-6 md:left-12">
            <motion.p {...fade(0.2)} className="text-xs tracking-[0.4em] uppercase mb-2" style={{ color: '#C8A44E' }}>
              {project.category}
            </motion.p>
        ...
        <div className="max-w-6xl mx-auto px-6 py-20">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            <motion.div {...fade(0.1)} className="lg:col-span-2">
              <p className="text-xs tracking-[0.4em] uppercase mb-4" style={{ color: '#C8A44E' }}>Overview</p>
              <p className="text-white/70 text-lg leading-relaxed">{project.overview}</p>
            </motion.div>

            {/* Services */}
            <motion.div {...fade(0.2)}>
              <p className="text-xs tracking-[0.4em] uppercase mb-4" style={{ color: '#C8A44E' }}>Services</p>
              <ul className="space-y-3">
                {project.services.map(s => (
                  <li key={s} className="flex items-center gap-3 text-sm text-white/60">
                    <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: '#00C46A' }} />
                    {s}
                  </li>
                ))}
              </ul>
          ...
          {/* Divider */}
          <div className="my-16 h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(200,163,78,0.15), transparent)' }} />
          ...
          {project.gallery && project.gallery.length > 0 && (
            <div>
              <motion.p
                ...
                className="text-xs tracking-[0.4em] uppercase mb-8" style={{ color: '#C8A44E' }}
              >
                Gallery
              </motion.p>
              <div className={`grid gap-4 ${project.gallery.length === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}>
                {project.gallery.map((url, i) => (
                  <motion.div
                    ...
                    style={{ border: '1px solid rgba(200,163,78,0.08)', aspectRatio: '16/9' }}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
                      ...
                      <span className="text-xs font-bold tracking-widest uppercase px-4 py-2 rounded-full"
                        style={{ border: '1px solid rgba(200,163,78,0.5)', color: '#C8A44E', background: 'rgba(11,15,12,0.7)' }}>
                        View Full
                      </span>
                    </div>
                  </motion.div>
          ...
          >
            <p className="text-xs tracking-[0.5em] uppercase mb-4" style={{ color: '#C8A44E' }}>Ready to Begin</p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <motion.button
                ...
                whileHover={{ scale: 1.03, boxShadow: '0 0 30px rgba(200,163,78,0.3)' }}
                whileTap={{ scale: 0.97 }}
                className="inline-flex items-center gap-3 px-8 py-4 rounded-full font-bold tracking-widest uppercase text-sm"
                style={{ background: 'linear-gradient(135deg, rgba(200,163,78,0.9), rgba(200,163,78,0.7))', color: '#0B0F0C', minHeight: '44px' }}
              >
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="inline-flex items-center gap-3 px-8 py-4 rounded-full font-bold tracking-widest uppercase text-sm"
                style={{ border: '1px solid rgba(200,163,78,0.25)', color: '#C8A44E', minHeight: '44px' }}
              >
